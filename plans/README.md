# K3s CI/CD Demo Application with Notes Feature

**Полный пример приложения с CI/CD пайплайном для K3s кластера и возможностью создания заметок**
...
- ✅ **Система управления заметками** с CRUD операциями и PostgreSQL
...
| `/api/notes` | POST | Создать заметку (JSON) |
| `/api/notes` | GET | Получить все заметки |
| `/api/notes/{id}` | GET | Получить заметку по ID |
| `/api/notes/{id}` | PUT | Обновить заметку |
| `/api/notes/{id}` | DELETE | Удалить заметку |
| `/api/notes/search` | GET | Поиск заметок |
